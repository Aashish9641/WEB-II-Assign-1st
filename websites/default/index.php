...
.ops {
 ...
 /* this is for added categoiry items */
...
.dropdown-content a:hover {
    background-color: aqua; /* Changing  the hover background color */
}

/* Show the dropdown menu on hover section */
...
if (isset($_SESSION['userLog'])) {
    ...
    if (!empty($userLog['name'])) {
		...
		// adding the add acution and edit auction
...
$disp->execute();
$caa = $disp 	->fetchAll(PDO::FETCH_ASSOC); // Feting into an associative array
		...
		<main>
			...
			<ul class="carList">
				...
				<li>
					...
					<img src="../c4.jpg" alt="car name">
					<article>
						<h2>Car model and make</h2>
						<h3>Car category</h3>
						<p>A car is a road vehicle used to transport passengers. It is also called car, which comes from the Greek word for "car" and the French word for "mobile phone". The name means "self-driving" because the cars do not need horses or other external energy sources to move. Cars usually have four wheels and are powered by an engine.</p>
...
$sql = "SELECT * FROM auction"; // auction table created on datbase
$stmt = $connection->prepare($sql); // connection
$stmt->execute(); // declaring
$auctions= $stmt->fetchAll(PDO::FETCH_ASSOC);// fetching all the items
